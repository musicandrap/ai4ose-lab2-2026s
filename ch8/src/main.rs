    ...
    impl SyncMutex for SyscallContext {
        ...
        fn semaphore_create(&self, _caller: Caller, res_count: usize) -> isize {
                ...
                .find(|(_, item)| item.is_none()).map(|(id, _)| id)
            {
                ...
                current_proc.sem_owners[id] = BTreeMap::new();
                current_proc.sem_avail[id] = res_count;
                id
            } else {
                current_proc.semaphore_list.push(Some(Arc::new(Semaphore::new(res_count))));
                current_proc.sem_owners.push(BTreeMap::new());
                current_proc.sem_avail.push(res_count);
        ...
        fn semaphore_up(&self, _caller: Caller, sem_id: usize) -> isize {
            ...
            // 更新 Allocation 矩阵：减少当前线程的持有计数
            // 因为 V 操作意味着释放资源
            let current_tid = unsafe { (*processor).current().unwrap().tid };
            if let Some(count) = current_proc.sem_owners[sem_id].get_mut(&current_tid) {
                if *count > 0 {
                    *count -= 1;
                }
            }
            ...
            if let Some(tid) = sem.up() {
                // 如果唤醒了等待线程，说明资源直接转交给了该线程
                // 更新 Allocation：增加被唤醒线程的持有计数
                *current_proc.sem_owners[sem_id].entry(tid).or_insert(0) += 1;
                // 更新 Need：被唤醒线程不再等待该资源
                current_proc.thread_waiting_for.remove(&tid);
                unsafe { (*processor).re_enque(tid); }
            } else {
                // 如果没有唤醒任何线程，说明资源被放回池中
                // 更新 Available：可用资源数 +1
                current_proc.sem_avail[sem_id] += 1;
            }
            0
        }
        ...
        fn semaphore_down(&self, _caller: Caller, sem_id: usize) -> isize {
            ...
            // 死锁检测：在尝试获取资源前，先检查这次请求是否会导致不安全状态
            if !current_proc.check_deadlock(tid, Resource::Semaphore(sem_id)) {
                // 如果检测到潜在死锁，拒绝请求并返回特定错误码
                return -0xDEAD;
            }
            ...
            if !sem.down(tid) { 
                // 获取失败，线程将进入阻塞态
                // 更新 Need：记录当前线程正在等待该信号量
                current_proc.thread_waiting_for.insert(tid, Resource::Semaphore(sem_id));
                -1 
            } else { 
                // 获取成功
                // 更新 Allocation：增加当前线程的持有计数
                *current_proc.sem_owners[sem_id].entry(tid).or_insert(0) += 1;
                // 更新 Available：可用资源数 -1
                current_proc.sem_avail[sem_id] -= 1;
                0 
        ...
        fn mutex_create(&self, _caller: Caller, blocking: bool) -> isize {
                ...
                .find(|(_, item)| item.is_none()).map(|(id, _)| id)
            {
                ...
                current_proc.mutex_owners[id] = None;
                id as isize
            } else {
                current_proc.mutex_list.push(new_mutex);
                current_proc.mutex_owners.push(None);
        ...
        fn mutex_unlock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            if let Some(tid) = mutex.unlock() {
                // 唤醒了等待线程，锁的所有权转移给它
                // 更新 Allocation：记录新的持有者
                current_proc.mutex_owners[mutex_id] = Some(tid);
                // 更新 Need：被唤醒线程不再等待
                current_proc.thread_waiting_for.remove(&tid);
                unsafe { (*processor).re_enque(tid); }
            } else {
                // 没有等待线程，锁变为空闲
                // 更新 Allocation：无持有者
                current_proc.mutex_owners[mutex_id] = None;
            }
            0
        }
        ...
        fn mutex_lock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            // 死锁检测：在请求锁之前检查安全性
            if !current_proc.check_deadlock(tid, Resource::Mutex(mutex_id)) {
                return -0xDEAD;
            }
            ...
            if !mutex.lock(tid) { 
                // 获取失败，阻塞
                // 更新 Need：记录当前线程正在等待该锁
                current_proc.thread_waiting_for.insert(tid, Resource::Mutex(mutex_id));
                -1 
            } else { 
                // 获取成功
                // 更新 Allocation：记录当前线程为持有者
                current_proc.mutex_owners[mutex_id] = Some(tid);
                0 
            }
        }
