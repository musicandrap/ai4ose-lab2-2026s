...
// 修改后：将倍率调大，确保有足够的空间存放 tcbs 数组和运行时栈帧
// 这里我们将每个 App 的预留空间乘以 4，确保万无一失
tg_linker::boot0!(rust_main; stack = (APP_CAPACITY * 4 + 2) * 8192);
// ========== 内核主函数 ==========
    ...
    match tcb.handle_syscall() {
        ...
        Event::UnsupportedSyscall(id) => {
            ...
            true
                ...
                if finish {
                log::info!("app{i} syscall statistics:");
                for (id, cnt) in tcb.syscall_stat().iter().enumerate() {
                    if *cnt > 0 {
                    log::info!("  syscall {} -> {}", id, cnt);
                        }
                    }
