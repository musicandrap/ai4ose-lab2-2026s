...
mod impls {
    use crate::{
        build_flags,
        // mmap 中先拼出权限字符串，再通过 parse_flags 转成 VmFlags
        parse_flags,
        fs::{read_all, FS},
    ...
    impl Memory for SyscallContext {
        ...
        fn mmap(
            ...
            // 只要求起始地址页对齐；len 允许非页对齐，后面会向上取整页数
            if addr & PAGE_MASK != 0 {
                return -1;
            }
            // prot 仅允许低三位 RWX，且不能全 0
            if prot & !0x7 != 0 || prot & 0x7 == 0 {
                return -1;
            }
            let current = PROCESSOR.get_mut().current().unwrap();
            // 按页向上取整，保证 mmap(start, len+1, ...) 能覆盖到完整页
            ...
            // 目标区间中只要有一页已映射，就拒绝本次映射，避免重叠覆盖
            for i in 0..page_count {
                let vpn = start_vpn + i;
                if current
                    .address_space
                    .translate::<u8>(vpn.base(), build_flags("U___V"))
                    .is_some()
                {
                    return -1;
                }
            }
            // 从基础权限 U___V 动态打开 R/W/X 三个位
            ...
            // 使用 map 分配匿名页并建立页表项，避免 map_extern 带来的连续物理页假设问题
            current.address_space.map(start_vpn..end_vpn, &[], 0, flags);
            // 页表更新后刷新 TLB，避免继续使用旧转换缓存
