...
extern "C" fn rust_main() -> ! {
    ...
    // 步骤 8：加载 ch8_usertest（返回 Process + Thread）
    let initproc = read_all(FS.open("ch8_usertest", OpenFlags::RDONLY).unwrap());
...
mod impls {
    ...
    impl SyncMutex for SyscallContext {
        ...
        fn mutex_lock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            // 死锁检测：同一线程重复加锁同一把阻塞互斥锁
            if current_proc.deadlock_detect_enabled {
                if let Some(owner) = current_proc.mutex_owners[mutex_id] {
                    if owner == tid {
                        return -0xDEAD;
                    }
                }
