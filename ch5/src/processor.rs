...
/// stride 调度算法中的 BigStride 常数
/// 根据算法要求：P.pass = BigStride / P.priority
/// 选择 0x7FFFFFFF (2^31-1) 作为一个适中值，避免溢出
...
impl Schedule<ProcId> for ProcManager {
    ...
    fn fetch(&mut self) -> Option<ProcId> {
        ...
        // 移除并返回最小 stride 的进程
        if let Some(pid) = min_pid {
            // 从就绪队列中移除该进程
            self.ready_queue.remove(min_idx);
            
            // 更新被调度进程的 stride
            if let Some(process) = self.tasks.get_mut(&pid) {
                // 计算 pass 值：BigStride / priority
                // 注意：优先级必须 >= 2，这里可以安全地进行整数除法
