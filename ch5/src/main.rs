...
fn map_portal(space: &AddressSpace<Sv39, Sv39Manager>) {
    ...
    mod impls {
        use crate::{
            build_flags, parse_flags, process::Process as ProcStruct, processor::ProcManager, Sv39, APPS,
            PROCESSOR,
        };
    ...
    impl Scheduling for SyscallContext {
        ...
        fn set_priority(&self, _caller: Caller, prio: isize) -> isize {
            ...
            let processor = PROCESSOR.get_mut();
    ...
    impl Memory for SyscallContext {
        ...
        fn mmap(
            ...
            for i in 0..page_count {
                let vpn = start_vpn + i;
                if current.address_space.translate::<u8>(vpn.base(), build_flags("U___V")).is_some() {
                    // 该虚拟页已被映射
                    return -1;
                }
            }
        ...
        fn munmap(
            ...
            for i in 0..page_count {
                let vpn = start_vpn + i;
                if current.address_space.translate::<u8>(vpn.base(), build_flags("U___V")).is_none() {
                    // 该虚拟页未被映射
                    return -1;
                }
            }
