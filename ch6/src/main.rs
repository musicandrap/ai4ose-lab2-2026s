...
mod impls {
    ...
    /// Stat 结构体定义（与 tg-syscall 中的定义保持一致）
    #[repr(C)]
    #[derive(Debug)]
    pub struct Stat {
    ...
    /// 可写权限标志
    const WRITEABLE: VmFlags<Sv39> = build_flags("UW_V");
    ...
    impl IO for SyscallContext {
        ...
        fn linkat(
            ...
            let current = PROCESSOR.get_mut().current().unwrap();
            if let Some(old_ptr) = current.address_space.translate(VAddr::new(oldpath), READABLE) {
                if let Some(new_ptr) = current.address_space.translate(VAddr::new(newpath), READABLE) {
                    // 读取旧文件路径
                    let mut old_string = String::new();
                    let mut raw_ptr: *mut u8 = old_ptr.as_ptr();
                    loop {
                        unsafe {
                            let ch = *raw_ptr;
                            if ch == 0 {
                    ...
                    let mut raw_ptr: *mut u8 = new_ptr.as_ptr();
                    loop {
                        unsafe {
                            let ch = *raw_ptr;
                            if ch == 0 {
                    ...
                    log::error!("newpath not readable");
                    -1
                }
            } else {
                log::error!("oldpath not readable");
                -1
            }
        }
        ...
        fn fstat(&self, _caller: Caller, fd: usize, st: usize) -> isize {
            ...
            if let Some(inode) = &file.inode {
                // 获取inode编号
                let ino = inode.inode_number() as u64;
                ...
                log::info!("fstat: fd={}, ino={}, mode={:?}, nlink={}", fd, ino, mode, nlink);
                ...
                if let Some(mut ptr) = current.address_space.translate::<Stat>(VAddr::new(st), WRITEABLE) {
                    ...
                    log::error!("fstat: st not writable");
                    -1
                }
            } else {
                // 对于没有inode的文件（如管道），返回错误
                log::error!("fstat: no inode for fd {}", fd);
                -1
            }
        }
    }
    ...
    impl Process for SyscallContext {
        ...
        fn spawn(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            // 从文件系统打开并读取ELF数据
            let elf_data = match FS.open(name, OpenFlags::RDONLY) {
                Some(fd) => read_all(fd),
                None => {
                    tg_console::log::error!("spawn: unknown app '{}'", name);
                    return -1;
                }
            };
            
            // 解析ELF文件
            let elf = match ElfFile::new(elf_data.as_slice()) {
                Ok(elf) => elf,
                Err(_) => {
                    tg_console::log::error!("spawn: invalid ELF file '{}'", name);
                    return -1;
                }
            };
            ...
            let child_proc = match ProcStruct::from_elf(elf) {
                Some(proc) => proc,
                None => {
                    tg_console::log::error!("spawn: failed to create process from ELF '{}'", name);
                    return -1;
                }
            };
    ...
    impl Memory for SyscallContext {
        ...
        ///
        /// 参数：
        /// - addr: 建议的映射起始地址（测试中为 0x10000000）
        /// - len: 映射长度（字节数）
        /// - prot: 保护标志（1=PROT_READ, 2=PROT_WRITE, 3=PROT_READ|PROT_WRITE）
        /// - flags: 映射类型（MAP_ANONYMOUS = 0 表示匿名映射）
        /// - fd: 文件描述符（匿名映射忽略）
        /// - offset: 文件偏移（匿名映射忽略）
        ///
        /// 返回：成功返回映射地址，失败返回 -1
        fn mmap(
            ...
            if addr % PAGE_SIZE != 0 || len % PAGE_SIZE != 0 {
                tg_console::log::info!("mmap: addr={:#x} or len={} not aligned, returning -1", addr, len);
                return -1;
            }
            
            // 2. 权限位合法性校验
            if prot == 0 || (prot & !0x7) != 0 {
                tg_console::log::info!("mmap: invalid prot={}, returning -1", prot);
                return -1;
            }
            
            // 3. 检查 prot=2 (只写不读)
            if prot == 2 {
                tg_console::log::info!("mmap: prot=2 (write-only) not supported, returning -1");
                return -1;
            }
            ...
            let start_page = VAddr::<Sv39>::new(start_addr).floor();
            let end_page = VAddr::<Sv39>::new(end_addr).ceil();
            
            // 4. 构建 VmFlags
            let readable = prot & 1 != 0;
            let writable = prot & 2 != 0;
            let executable = prot & 4 != 0;
            
            let flags_str = if readable && writable && executable {
                "URWX_V"
            ...
            let vm_flags = build_flags(flags_str);
            
            // 5. 分配物理页并建立映射
            let pages_needed = end_page.val() - start_page.val();
            let page_ptr = Sv39Manager::page_alloc::<u8>(pages_needed);
            
            unsafe {
                core::ptr::write_bytes(page_ptr as *mut u8, 0, pages_needed * PAGE_SIZE);
            }
            ...
            tg_console::log::info!(
                "mmap: addr = {:#x}, len = {}, pages = {}, flags = {}, success",
                start_addr, len, pages_needed, flags_str
            );
        ...
        fn munmap(&self, _caller: Caller, addr: usize, len: usize) -> isize {
            ...
            const PAGE_SIZE: usize = 1 << Sv39::PAGE_BITS;
            if addr % PAGE_SIZE != 0 {
                tg_console::log::info!("munmap: addr not aligned, returning -1");
                return -1;
            }
            ...
            // 3. 取消映射
            current.address_space.unmap(start_page..end_page);
            
            tg_console::log::info!(
                "munmap: addr = {:#x}, len = {}, pages = {}, success",
                start_addr, len,
                end_page.val() - start_page.val()
            );
